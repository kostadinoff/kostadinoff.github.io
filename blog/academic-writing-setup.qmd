...
Every few months another editorial asks whether artificial intelligence is going to change academic writing. It is the wrong tense. The change is not coming — it arrived, quietly, somewhere in the last three years, while journals were still [drafting policies about it](https://doi.org/10.1038/d41586-023-00191-1) and departments were still holding seminars asking whether it would happen at all.
...
What changed is **drafting and editing**. The blank page is no longer expensive. Neither is the third revision of a Methods paragraph, nor translating your own Bulgarian-shaped English into something a Cambridge copy-editor would not flinch at. For a non-native speaker in particular — and most of the world's researchers are non-native speakers — this is not a small convenience. The costs of doing science in a second language are real and [measurable](https://doi.org/10.1371/journal.pbio.3002184): more time per paper, more rejections on language grounds, more rounds of revision, none of them related to the quality of the ideas. What has been lifted is the tax I described at the outset.
...
The practical consequence is that fluency has stopped being a signal. For a long time, well-written papers were, on average, better papers — not because good prose causes good science, but because the same care and the same seniority produced both. That correlation is now broken. When everyone's Introduction reads well, polished prose tells you nothing about the study behind it.
...
├── scripts/
│   ├── 00_run_all.R       # runs 01–04 in order: the reproducibility test
│   ├── 01_data_prep.R
│   ├── 02_analysis.R
│   ├── 03_figures.R
│   └── 04_tables.R
...
├── manuscript/
│   ├── manuscript.md      # prose + generated output, no code
│   ├── references.bib
│   └── supplementary.md
├── presentation/
│   └── presentation.typ   # Typst talk — communication counts too
...
Two choices in that tree are deliberate and easy to miss. The manuscript is plain Markdown, not a computational notebook: the analysis lives in the numbered scripts, and the manuscript only *consumes* their output — tables and figures pasted or included, never code executed at render time. Quarto renders it exactly the same, and keeping computation out of the prose file means the paper can never quietly disagree with the analysis. The second is the `presentation/` folder, present from the first day, holding a [Typst](https://typst.app) talk. A finding that never leaves the paper has done half its job; building the slides in the same repository, against the same figures, keeps the talk faithful to the analysis and makes communication a first-class output rather than a scramble the night before a conference.
...
Make `data/raw/` read-only. On a real Linux filesystem — including WSL's own ext4 home — one command does it:

```bash
chmod -R a-w data/raw
```

(One caveat worth knowing: on a Windows-mounted path under `/mnt/c`, or an exFAT or cloud-synced drive, that permission bit silently does not stick — there the read-only rule is a discipline, not a lock, so keep such data off those drives or set it read-only from the host.) It costs nothing and it prevents the single most common irrecoverable mistake in applied research — silently overwriting the source data with a partially cleaned version.
:::
...
**Reproducible research.** Not a virtue; an efficiency. The main beneficiary of a reproducible pipeline is you, at revision time, when a request for a subgroup analysis costs an hour instead of a fortnight.
...
**Publication-quality by default.** Figures are written to PNG through the AGG device (`ragg`) at print resolution with a consistent theme, so the figure I look at while exploring is the figure that goes into the paper. No last-minute re-export at 2 a.m.

**Automatically reported.** Numbers reach the manuscript as generated output, not by transcription. `Median age was 62.4 years` is not typed; it is computed by a script and carried in. Typing numbers into prose is a step that has no upside and a [well-documented failure rate](https://doi.org/10.3758/s13428-015-0664-2) — in psychology, where this has been checked at scale, roughly half of published papers contain at least one inconsistent statistic.
