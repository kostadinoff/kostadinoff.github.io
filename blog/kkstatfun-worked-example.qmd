...
Every applied statistics package makes a claim about what analysis *is*. Most of them claim it is a sequence of tests. You hand over data, you get back a *p*-value, and the interpretation is left as an exercise. [`kkstatfun`](../kkstatfun.qmd) makes a different claim, and this post is an attempt to demonstrate it rather than assert it: that an analysis is a chain of decisions, that most of those decisions should be made once and reused forever, and that a good tool makes the uncomfortable ones visible instead of hiding them behind a default.
...
Then, in §12, the part that actually pays for the package: the public-health workflow it sits inside — how the data gets out of the National Health Insurance Fund and the Ministry of Health in the first place, what happens to it before an estimate is legitimate, and which parts of that I now hand to a machine.
...
This is the philosophical point of the whole package in miniature. The function did not silently fix my figure, and it did not refuse to run. It told me precisely what was wrong, in units I can act on, and left the decision where it belongs.
...
Stratification handles one confounder. For several at once the package gives three routes with different assumptions, and — the point — the same interface.
...
The `early_abx` row moves from `r rr |> filter(term == "early_abx", model_type == "univariate") |> pull(risk_ratio) |> round(2)` univariately to `r rr |> filter(term == "early_abx", model_type == "multivariable") |> pull(risk_ratio) |> round(2)` adjusted. That single sign flip is the entire post.
...
And, because most of my data is Bulgarian administrative data, the registry helpers — `extract_egn_info()` parses date of birth, sex, age and birth region out of an EGN; `kk_std_rates()` and `kk_smr()` do direct and indirect standardisation; `kk_nb_scan()` looks for space–time clusters under overdispersion.

## 11. The philosophy, stated plainly

Having shown it, let me name it. Four commitments, in the order they matter.

**Tidy data in, tidy data out.** Every function takes a data frame first and returns a tibble. Never a print-only object you have to scrape, never a list of matrices, never a `summary()` you parse with regular expressions. This is why §3 could pipe a 2×2 result straight into a plot, and why any result in this post could be written to a CSV and pasted into a manuscript without an intermediate step.
...
## 12. Where this actually gets used: public-health analysis on national data

None of the above is hypothetical tooling. The synthetic cohort is a teaching device; the real work is population-level, and the analysis layer is the *last* step of it. A public-health question in Bulgaria — how many people are treated for a condition, where, at what cost, and how that changed after a policy — is answered from administrative data held by two institutions, and getting it out of them is most of the job.

### Asking for the data

The **National Health Insurance Fund** (НЗОК / NHIF) publishes a good deal: hospitalisations by clinical pathway (КП), outpatient activity, reimbursed medicines by ATC code, dialysis, IVF, dental. It publishes it as Excel and PDF attachments scattered across a website, in formats that change between years.

The **Ministry of Health** (МЗ) holds what the NHIF does not, most importantly the **National Health Information System** (НЗИС) — the electronic prescription, referral and hospitalisation records that now cover essentially the whole system, including activity that never generates an insurance claim. Almost none of it is published. For that you write to the Ministry.

Both routes run through the Access to Public Information Act (ЗДОИ), and after a fair number of these I would say the request itself is a skill with rules:

- **Ask for aggregates, never person-level records.** Counts and sums by КП or ATC code, month, region (област), age band and sex. This is not only the lawful form of the request — it removes the institution's best reason to refuse, since no personal data means no data-protection ground for saying no.
...
- **Say the format in the request.** Write `.csv` or `.xlsx` explicitly. Otherwise the answer arrives as a scanned PDF of a printed table, and the institution has complied.
- **Ask for the codelist alongside the data.** The version of the КП or ATC list in force in each year of the period — otherwise you will spend a week reconstructing which code was renumbered when.
- **Anticipate the aggregation refusal.** Where cells are small, propose the suppression rule yourself in the request (for example, cells below five reported as "<5"). It converts a likely refusal into a qualified release.
...
The reply usually arrives within the statutory two weeks, and it is usually incomplete in some interesting way. That is normal, and it is why the next step exists.

### The pipeline

What arrives is never clean. Merged header cells, footer totals rows, comma decimal separators, Cyrillic and Latin mixed within a column, MKB codes that Excel has helpfully converted to dates, and a schema that changes between years. So the pipeline is four numbered scripts and one rule:
...
Three conventions do most of the work. Money is stored in **both BGN and EUR** at the fixed rate of 1.95583, which matters enormously across the 2025–2026 adoption boundary, where a careless join double-converts a column and nobody notices. CSV output is **UTF-8 with BOM**, so Cyrillic survives the round trip through Excel. And every dataset ships a `data-dictionary.csv` with one row per column — Bulgarian label, English label, type, unit, codelist, the original NHIF field name, and the caveats. The dictionary is the artefact that survives schema drift; the code is comparatively disposable.
...
At this point the file is a tidy panel of counts and costs by region, year, age band and code, and everything in §§3–9 applies — with the population-health functions doing the work the clinical ones did above. Rates per person-time with exact Poisson intervals from `kk_incidence_rate()`. Direct age-standardisation against the European Standard Population with `kk_std_rates()`, or `kk_std_rates_ci()` where the counts are sparse enough that the Gamma interval matters. Indirect standardisation and an SMR by district with `kk_smr()`. Trends over the period with `kk_apc()`, and the spatial-temporal outlier hunt with `kk_nb_scan()`.

And when the question is about a policy — a new reimbursement rule, a pathway price change, a programme rolled out in some districts before others — it is §8 verbatim: `kk_its()` on the national monthly series, `kk_did()` or `kk_did_staggered()` across districts, with `kk_event_study()` to show whether the parallel-trends story holds. Administrative data is unusually well suited to those designs, because the intervention really was switched on by a rule with a date, and the date is in the *Държавен вестник*.

That is the whole argument for building the package. The interesting part of a public-health analysis is deciding what to ask an institution for and what the resulting estimate can support. The part in between should be a function call I have already tested.

### Where AI fits, and where it does not

I run this with [Claude Code](https://claude.com/claude-code), and the arrangement that made it useful rather than merely fast is **skills**: plain Markdown files in `~/.claude/skills/`, one per recurring task, each stating the conventions for that task. `nhif-data-pipeline` holds the directory layout, the currency rule, the encoding rule and the data-dictionary schema. `kkstatfun-epi` says to reach for `kk_*` functions over `epitools` or `gtsummary` and how they compose. `bibliography-management` holds the citation rules below. `humanizer-bg` strips the calques out of Bulgarian prose.

The distinction I would insist on is this: **the conventions are written by me, in version control, once. The model applies them.** That is the opposite of asking a chatbot how to clean health data. The AI is fast at the mechanical middle — parsing a malformed sheet, writing the ingest function for the twelfth source file, drafting the data dictionary from the column names, catching the year where the schema changed, drafting the ЗДОИ request in the register's own idiom. It is not the thing deciding what the currency rule is, whether a join is legitimate, or what an estimate means. Every number it produces gets checked against the source, because a fluent wrong answer is the characteristic failure mode and it costs nothing to generate.

The small tools around it matter more than they sound. `anydoc` converts the legacy `.doc` and `.xls` files institutions still send into something readable — pandoc will silently emit binary garbage for an `.xls` rather than failing, which is exactly the kind of error that ends up in a published table. DBeaver is where I sanity-check a join before writing analysis code against it. Local models via Ollama take the bulk mechanical passes — summarising, extracting, first-draft translation — for text that should not leave the machine at all. And the whole thing is in Git, because the value of version control here is not collaboration but a record of my own reasoning: the commit that dropped 1,400 rows says why it dropped them.
...
`getbib` populates the `abstract` field automatically, and that field is the point. **Each entry stores what the source actually found**, so when I need support for a claim I search my own bibliography and what comes back is a finding, not a title. A claim followed by `[@key]` has to be supported by the abstract recorded in that entry — which is the single most effective guard I have against citing a paper for something it does not say, a failure that fluent drafting makes easier rather than harder. Non-PubMed sources — NHIF releases, legal acts, Bulgarian-language reports — have no abstract to fetch, so I write the findings statement by hand or the entry does not get cited.
